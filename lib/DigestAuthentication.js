...
var DigestAuthentication = /** @class */ (function () {
    ...
    DigestAuthentication.prototype.calculateResponse = function (body) {
        ...
        var ha1 = crypto_js_1.MD5(this.username + ":" + this.realm + ":" + this.password);
        if (this.qop === "auth") {
            // HA2 = MD5(A2) = MD5(method:digestURI)
            ha2 = crypto_js_1.MD5(this.method + ":" + this.uri);
            // response = MD5(HA1:nonce:nonceCount:credentialsNonce:qop:HA2)
            this.response = crypto_js_1.MD5(ha1 + ":" + this.nonce + ":" + this.ncHex + ":" + this.cnonce + ":auth:" + ha2);
        }
        else if (this.qop === "auth-int") {
            // HA2 = MD5(A2) = MD5(method:digestURI:MD5(entityBody))
            ha2 = crypto_js_1.MD5(this.method + ":" + this.uri + ":" + crypto_js_1.MD5(body ? body : ""));
            // response = MD5(HA1:nonce:nonceCount:credentialsNonce:qop:HA2)
            this.response = crypto_js_1.MD5(ha1 + ":" + this.nonce + ":" + this.ncHex + ":" + this.cnonce + ":auth-int:" + ha2);
        }
        else if (this.qop === undefined) {
            // HA2 = MD5(A2) = MD5(method:digestURI)
            ha2 = crypto_js_1.MD5(this.method + ":" + this.uri);
            // response = MD5(HA1:nonce:HA2)
            this.response = crypto_js_1.MD5(ha1 + ":" + this.nonce + ":" + ha2);
        }
    };
