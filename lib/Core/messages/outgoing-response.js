...
function constructOutgoingResponse(message, options) {
    ...
    var response = Utils_1.Utils.buildStatusLine(options.statusCode, options.reasonPhrase);
    ...
    if (options.statusCode > 100 && !message.parseHeader("to").hasParam("tag")) {
        ...
        if (!toTag) {
            ...
            toTag = Utils_1.Utils.newTag(); // FIXME: Utils.newTag() currently generates random tags
        }
        toHeader += ";tag=" + toTag;
    }
    toHeader += CRLF;
    ...
    var supportedHeader = SIPMessage_1.getSupportedHeader(message);
    // FIXME: TODO: needs review...
    var userAgentHeader = "";
    ...
    if (options.body) {
        ...
        response += "Content-Length: " + Utils_1.Utils.str_utf8_length(options.body.content) + CRLF + CRLF;
