...
const Modifiers = tslib_1.__importStar(require("../web/modifiers"));
function defer() {
    const deferred = {};
...
class SessionDescriptionHandler extends events_1.EventEmitter {
    ...
    getDescription(options = {}, modifiers = []) {
        ...
        return Promise.resolve().then(() => {
            ...
            const error = new exceptions_1.SessionDescriptionHandlerError("createOffer failed");
    ...
    setDescription(sessionDescription, options = {}, modifiers = []) {
        ...
        return Promise.resolve().then(() => {
            ...
            .catch((e) => {
            const error = new exceptions_1.SessionDescriptionHandlerError("The modifiers did not resolve successfully");
            ...
            const error = new exceptions_1.SessionDescriptionHandlerError("setDescription");
            this.emit("peerConnection-setRemoteDescriptionFailed", error);
            throw error;
        }).then(() => {
            if (this.peerConnection.getReceivers) {
    ...
    createOfferOrAnswer(RTCOfferOptions = {}, modifiers) {
        ...
        return pc[methodName](RTCOfferOptions).catch((e) => {
            ...
            const error = new exceptions_1.SessionDescriptionHandlerError("peerConnection-" + methodName + "Failed");
            ...
            const error = new exceptions_1.SessionDescriptionHandlerError("peerConnection-SetLocalDescriptionFailed");
            ...
            const error = new exceptions_1.SessionDescriptionHandlerError("createOfferOrAnswer");
            this.logger.error(error.toString());
            throw error;
        });
    }
    ...
    acquire(constraints) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const error = new exceptions_1.SessionDescriptionHandlerError("unable to acquire streams");
            this.logger.error(error.message);
            throw error;
        }).then((streams) => {
            ...
            .catch((e) => {
            const error = new exceptions_1.SessionDescriptionHandlerError("error removing streams");
            this.logger.error(error.message);
            throw error;
        }).then((streams) => {
            try {
            ...
            .catch((e) => {
            const error = new exceptions_1.SessionDescriptionHandlerError("error adding stream");
            this.logger.error(error.message);
            throw error;
        });
    }
