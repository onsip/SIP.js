...
var SimpleUser = /** @class */ (function () {
            ...
            onConnect: function () {
                ...
                if (_this.registerer && _this.registerRequested) {
                    ...
                    _this.registerer.register()
                        .catch(function (e) {
                        _this.logger.error("[" + _this.id + "] Error occurred registering after connection with server was obtained.");
            ...
            onDisconnect: function (error) {
                ...
                if (_this.session) {
                    ...
                    _this.hangup() // cleanup hung calls
                        .catch(function (e) {
                        _this.logger.error("[" + _this.id + "] Error occurred hanging up call after connection with server was lost.");
                        _this.logger.error(e.toString());
                    });
                }
                if (_this.registerer) {
                    ...
                    _this.registerer.unregister() // cleanup invalid registrations
                        .catch(function (e) {
                        _this.logger.error("[" + _this.id + "] Error occurred unregistering after connection with server was lost.");
                        _this.logger.error(e.toString());
                    });
                }
    ...
    SimpleUser.prototype.sendDTMF = function (tone) {
        ...
        return this.session.info({ requestOptions: requestOptions })
            .then(function () { return; });
    };
    /**
     * Send a message.
    ...
    SimpleUser.prototype.initSession = function (session, referralInviterOptions) {
        ...
        this.session.delegate = {
            onInfo: function (info) {
                ...
                // No delegate
                if (!_this.delegate || !_this.delegate.onCallDTMFReceived) {
                    info.reject();
                    return;
                }
                ...
                info
                    ...
                    if (_this.delegate && _this.delegate.onCallDTMFReceived) {
                        ...
                        _this.delegate.onCallDTMFReceived(tone, duration);
                    }
                })
                    .catch(function (error) {
                    _this.logger.error(error.message);
